document.addEventListener('DOMContentLoaded', function (e) {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return `<div class="btn-group">
                        <button class="btn btn-xs btn-sm btn-info examination-up" data-id="${row.admit_card_id}"><i class="fa fa-eye"></i> Verify</button>
    ...
    $(document).on('click', '.examination-up', function () {
        var admit_card_id = $(this).data('id');
        // alert(admit_card_id);
        $.AryaAjax({
            url: 'website/verify-examination-data',
            data: { admit_card_id: admit_card_id }
        }).then((res) => {
            // log(res);
            if (res.status) {
                myModel('Correction Form', res.html, 'website/update-examination-data').then((res) => {
                    // log(res);
                    if (res.status) {
                        mySwal('Success', 'Admit Generate Successfully..', 'success');
                        $("#list-admit-card").DataTable().ajax.reload();
                        ki_modal.modal('hide');
                    }
                });
            }
