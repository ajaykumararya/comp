...
window.onscroll = function () {
  ...
  const elementOffsetTop = counterElement.offsetTop;
  const elementHeight = getAbsoluteHeight(counterElement);
  const windowHeightDouble = 2 * window.innerHeight;

  if (window.pageYOffset > elementOffsetTop + elementHeight - windowHeightDouble && statNotUpdating) {
    statNotUpdating = false;
...
const imageObserverConfig = { rootMargin: "0px 0px 50px 0px", threshold: 0 };
let loadedImages = 0;
...
function scrollNews(element, duration) {
  if (!element) return;
  ...
  firstChild.style.cssText = `margin-top: -${height}px; transition: all ${duration}s;`;

  setTimeout(() => {
    firstChild.remove();
    scrollNews(element, duration);
  }, duration * 1000 + 500);
}
...
// const cookieToast = new bootstrap.Toast(document.getElementById("cookieToast"), {
//   autohide: false,
// });
// cookieToast.show();
const newContainer = document.querySelector("#news-container");
if (newContainer)
  scrollNews(newContainer, 2.5);
...
function handleCardHover(event) {
  ...
  const rotateX = ((clientY - offsetTop) / clientHeight) * ROTATE_THRESHOLD * 2 - ROTATE_THRESHOLD;
  const rotateY = ROTATE_THRESHOLD - ((clientX - offsetLeft) / clientWidth) * ROTATE_THRESHOLD * 2;

  card.style.transform = `perspective(${clientWidth}px) rotateX(${rotateX.toFixed(2)}deg) rotateY(${rotateY.toFixed(2)}deg) scale3d(1, 1, 1)`;
}

function resetCardStyles(event) {
  card.style.transform = `perspective(${event.currentTarget.clientWidth}px) rotateX(0deg) rotateY(0deg)`;
}

function sliderAuto(glider, interval) {
...
const recentCenters = document.querySelector("#recentCenters");
const testimonial = document.querySelector("#testimonials");
if (recentCenters) {
  const recentCentersGlider = new Glider(recentCenters, {
    slidesToShow: 3,
    slidesToScroll: 1,
    scrollLock: true,
    draggable: true,
    duration: 2,
    responsive: [
      { breakpoint: 480, settings: { slidesToShow: 2, slidesToScroll: "auto" } },
      { breakpoint: 600, settings: { slidesToShow: 3, slidesToScroll: "auto" } },
      { breakpoint: 1024, settings: { slidesToShow: 6, slidesToScroll: "auto" } },
    ],
  });
  sliderAuto(recentCentersGlider, 2000);
}
if (testimonial) {
  const testimonialsGlider = new Glider(testimonial, {
    slidesToShow: 3,
    slidesToScroll: 1,
    scrollLock: true,
    draggable: true,
  ...
  sliderAuto(testimonialsGlider, 2000);
}
